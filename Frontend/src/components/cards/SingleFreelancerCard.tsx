      ...
      <div className="d-flex align-items-center">
        ...
        <div className="flex-grow-1">
          ...
          <div className="d-flex align-items-center mt-1">
            <div className="me-3">{renderStars(freelancer.rating)}</div>

            <div className="me-3 d-flex align-items-center mx-2">
              <i className="bi bi-chat-left-text text-primary me-1"></i>
              <span className="fw-medium">{freelancer.reviews} Reviews</span>
            </div>

            <Badge bg="danger" className="px-3 py-2">
              <span className="fw-bold fs-5">${freelancer.hourlyRate}</span>
              <small className="ms-1">{freelancer.currency}/hr</small>
            </Badge>
          </div>

          {/* Categories */}
          <div className="mt-2">
            {freelancer.categories.split(',').map((category, index) => (
              <Badge
                key={index}
                bg="light"
                text="dark"
