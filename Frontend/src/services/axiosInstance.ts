...
export const refreshToken = async (): Promise<string | null> => {
  try {
    ...
    const newAccessToken: string | null = response?.data?.accessToken ?? null
    if (!newAccessToken) throw new Error('Invalid response from refresh token')

    // 🟢 Cập nhật token mới vào Zustand
    useAuthStore.setState((state) => ({
      user: state.user ? { ...state.user, accessToken: newAccessToken } : null,
    }))
